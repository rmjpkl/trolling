        ...
        function dataItemTemplatemalam2(item) {
    if (item.lokasi && item.rupam && item.tanggal && item.jam) {
        ...
        async function renderDatamalam2() {
            const wrapperDOM = document.getElementById('wrappermalam2');
            const loadingDOM = document.getElementById('loadingmalam2');
            loadingDOM.style.display = 'block'; // Tampilkan spinner
            try {
                const data = await getDatamalam2();
                wrapperDOM.innerHTML = data.map(item => dataItemTemplatemalam2(item)).join('');
            } catch (error) {
