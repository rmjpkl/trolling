        ...
        function dataItemTemplatemalam1(item) {
    if (item.lokasi && item.rupam && item.tanggal && item.jam) {
        ...
        async function renderDatamalam1() {
            const wrapperDOM = document.getElementById('wrappermalam1');
            const loadingDOM = document.getElementById('loadingmalam1');
            loadingDOM.style.display = 'block'; // Tampilkan spinner
            try {
                const data = await getDatamalam1();
                wrapperDOM.innerHTML = data.map(item => dataItemTemplatemalam1(item)).join('');
            } catch (error) {
